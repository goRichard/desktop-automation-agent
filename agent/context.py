...
from skills.registry import find_matching_skill, find_matching_skill_async, get_skills_summary
...
_PLAN_FIRST_CONSTRAINT = (
    ...
    "3. **失败立即上报**：工具调用返回错误时，立即停止执行并截图报告用户，**禁止**自行尝试替代方案、重试、或切换到其他步骤\n"
    "4. **禁止额外操作**：不执行计划以外的任何操作，即使你认为有更好的方法\n"
    "5. **工具选择约束（最重要）**：\n"
    "   - 如果计划步骤中指定了工具名（如 `click`、`find_and_click`、`run_actions`），**必须使用该工具**\n"
    "   - **禁止**使用计划未指定的工具，即使用户认为该工具更合适\n"
    ...
    "**违规检测**：如果你发现自己使用了计划未指定的工具，立即停止并重新按照计划执行。"
)
