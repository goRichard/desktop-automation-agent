...
`external_side_effect`。这类步骤默认要求确认，但可由受控 Skill 显式设置
`policy.requireConfirmation: false`。`send-email@2.0.1` 已采用该策略，不会生成或执行
`user.confirm`；该名称是 Skill 控制动作，不是 Agent 工具。该 Skill 使用 Classic Outlook Adapter：
...
当前基线包含 105 项自动化测试。完整仓库的 `ruff check .` 尚有旧 CLI、工具和视觉评估
