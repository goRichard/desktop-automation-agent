...
@tool(description="等待指定秒数。用于 UI 操作之间需要等待页面/窗口响应的场景。seconds 为等待时长（支持小数，如 0.3、1.5），最大不超过 30 秒。")
...
@tool(description="读取文本文件内容。file_path 为文件绝对路径，encoding 默认 utf-8。返回文件内容字符串。")
...
@tool(description="将文本内容写入文件。file_path 为文件路径，content 为写入内容，append=true 时追加而非覆盖。父目录不存在时自动创建。")
def write_file(
    file_path: str,
    content: str,
...
@tool(description="列出目录中的文件和子目录。dir_path 为目录路径，pattern 为可选的 glob 过滤模式（如 '*.txt'）。")
...
@tool(description="执行系统命令（PowerShell）。command 为要执行的命令字符串，直接写命令即可（如 'Get-Process'、'$p=\"C:\\temp\"; Get-ChildItem $p'），不需要加 powershell 前缀。返回命令输出。注意：谨慎使用，避免执行危险命令。")
...
@tool(description="将文字内容写入剪贴板。text 为要写入的文字内容。")
